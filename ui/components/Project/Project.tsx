...
function Project({ p_id }: Props) {
  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-[#444D61] to-[#5689C0]">
        <AiOutlineCloseCircle className="text-6xl text-red-600 mb-4" />
        <Typography variant="h4" className="text-white mb-4">Project not found</Typography>
        <Typography variant="body1" className="text-white mb-8">
          It seems like the project you{`'`}re looking for doesn{`'`}t exist. Please go back to the projects page to explore other projects.
        </Typography>
        <Button
          variant="contained"
          color="primary"
  ...
  return (
    <Card className="max-w-3xl mx-auto my-6 shadow-lg bg-gradient-to-r from-[#444D61] via-[#5689C0] to-[#75E2FF]">
      ...
      <CardContent>
        <Typography variant="h5" className="font-semibold text-lg mb-2 text-white">
          <FiZap className="inline-block mr-2 text-xl" />
          {project.name}
        </Typography>

        <Typography variant="body2" color="text.secondary" className="mb-2 text-white">
        ...
        <Typography variant="body2" color="text.secondary" className="mb-4 text-white">
        ...
        <div className="mb-4">
          <Typography variant="body2" color="text.secondary" className="mb-1 text-white">
            <strong>Technologies:</strong>
          </Typography>
          <Box className="flex flex-wrap gap-2">
            {project.technologies.map((tech, index) => (
              <Chip key={index} label={tech} variant="outlined" size="small" className="text-white border-white" />
            ))}
          </Box>
        </div>
        ...
        <div className="mb-4">
          ...
          <Box className="flex flex-wrap gap-2">
            {project.overview.keyFeatures.map((feature, index) => (
              <Chip key={index} label={feature} variant="outlined" size="small" className="text-white border-white" />
            ))}
          </Box>
        </div>
